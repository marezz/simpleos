...
export default {
  ...
  methods: {
    ...
    dfs(ctx, i, j, oldColor, newColor){
      ...
      var px = this.rgbToHex(pixelData[0],pixelData[1],pixelData[2]);
      
      if (i < 0 || j >= n || j < 0 || i >= m || px != oldColor){
        return
      } else { 
        //ctx.putImageData(imageData,i,j);
        ctx.fillRect(i,j,1,1);
        this.teste++;
        this.dfs(ctx, i+1, j, oldColor, newColor);
        this.dfs(ctx, i-1, j, oldColor, newColor);
        this.dfs(ctx, i, j+1, oldColor, newColor);
        this.dfs(ctx, i, j-1, oldColor, newColor);
      }
    },
    */
    bucket(ctx, row, col, oldColor, newColor, fillStack) {
      ...
      //const imagedata = ctx.getImageData(0, 0, m, n);
    ...
    floodFill(i, j, newColor) {
      ...
      var fillStack = [];
      var oldColor = ctx.getImageData(i, j, 1, 1).data;
      oldColor = this.rgbToHex(oldColor[0], oldColor[1], oldColor[2]);

      if (oldColor == newColor) {
        return
      }
      //this.dfs(ctx, i, j, oldColor, newColor);
      //console.log('chamou');
      var startTime = new Date();
      this.bucket(ctx, i, j, oldColor, newColor, fillStack);
      var endTime = new Date();
      var timeDiff = (endTime - startTime) / 1000;
      console.log('seconds: ' + timeDiff);
    },
    fillLocation() {
      if (document.getElementById('tryBucket').checked) {
        while (document.getElementById('tryBucket').checked) {
          //console.log('chamou');
          var canvas = document.getElementById('canvas');
          canvas.addEventListener("click", (evt) => {
            console.log('teste')
            var mousePos = this.getMousePos(canvas, evt);
            this.floodFill(Math.round(mousePos.x), Math.round(mousePos.y), this.color);
          }, false);
        }
      }
    },
